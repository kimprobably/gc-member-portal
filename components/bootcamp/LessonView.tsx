...
const LessonView: React.FC<LessonViewProps> = ({ 
  ...
  const isChecklistHub = lesson.id.endsWith(':checklist') || lesson.embedUrl === 'virtual:checklist';
      ...
      <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            {currentWeek && <span className="text-slate-400 dark:text-slate-600 text-[10px] font-bold uppercase tracking-widest">• {currentWeek.title}</span>}
          </div>
      ...
      {isChecklistHub ? (
        <div className="space-y-4">
          {weekTasks.map((task, idx) => {
            ...
            return (
              <div key={task.id} className={`bg-white dark:bg-slate-900 rounded-2xl border transition-all ${isActive ? 'border-brand-500 shadow-xl' : 'border-slate-200 dark:border-slate-800'}`}>
                <button onClick={() => setActiveTaskId(isActive ? null : task.id)} className="w-full flex items-center gap-4 p-5 text-left">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 border text-xs font-bold ${isDone ? 'bg-green-500 border-green-500 text-white' : (isActive ? 'bg-brand-500 border-brand-500 text-white' : 'bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-400')}`}>
                    {isDone ? <Check size={14} strokeWidth={3} /> : idx + 1}
                  </div>
                  <h4 className={`flex-1 text-sm font-bold ${isActive ? 'text-slate-900 dark:text-white' : 'text-slate-600 dark:text-slate-400'}`}>{cleanTitle(task.text)}</h4>
                  {isActive ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
                </button>
                {isActive && (
                  <div className="border-t border-slate-100 dark:border-slate-800 p-6 space-y-6">
                    <div className="grid md:grid-cols-2 gap-4">
                      <textarea placeholder="Observations..." value={taskNotes[task.id] || ''} onChange={(e) => onUpdateNote(task.id, e.target.value)} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs outline-none min-h-[100px]" />
                      <div className="space-y-3">
                        <input type="text" placeholder="Proof URL..." value={proofOfWork[task.id] || ''} onChange={(e) => onUpdateProof(task.id, e.target.value)} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs outline-none" />
                        <button onClick={() => onToggleItem(task.id)} className={`w-full py-4 rounded-xl font-bold text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 ${isDone ? 'bg-green-500 text-white' : 'bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900'}`}>
                          {isDone ? <CheckCircle size={14} /> : <Target size={14} />} {isDone ? 'Step Verified' : 'Mark Complete'}
                        </button>
                      </div>
                    </div>
                  </div>
        ...
        <div className="animate-slide-in">
          <div className="mb-10">
            {isTextContent ? (
              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-10 md:p-16 shadow-xl text-sm leading-relaxed text-slate-700 dark:text-slate-300 whitespace-pre-wrap" dangerouslySetInnerHTML={{ __html: lesson.embedUrl.replace(/^text:\s*/, '') }} />
            ) : isPickaxe && pickaxeId ? (
              <div className="w-full min-h-[800px] bg-white dark:bg-slate-900 rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-inner flex flex-col">
                <div key={pickaxeId} id={pickaxeId} className="w-full flex-grow min-h-[700px]"></div>
                <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 flex items-center justify-center gap-2">
                  ...
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Powered by Modern Agency Sales</span>
                </div>
              </div>
            ) : isAistudio ? (
              <div className="bg-slate-900 p-16 rounded-2xl border border-slate-800 shadow-2xl flex flex-col items-center justify-center text-center gap-6">
                ...
                <a href={lesson.embedUrl} target="_blank" rel="noopener noreferrer" className="bg-brand-500 text-white px-8 py-4 rounded-xl font-bold text-xs uppercase tracking-widest flex items-center gap-3">
            ...
            <div className="bg-slate-900 p-10 rounded-3xl flex flex-col md:flex-row items-center justify-between gap-8 border border-slate-800 shadow-2xl">
               <div className="flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-400">Log your progress in the project workflow.</p>
                  </div>
               </div>
               <button 
                onClick={() => onSelectLesson({ id: `${currentWeek?.id}:checklist`, title: 'Tasks', embedUrl: 'virtual:checklist' })} 

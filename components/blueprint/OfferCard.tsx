    ...
    <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm dark:shadow-none overflow-hidden">
      ...
      <div className="p-6 sm:p-8">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-4">
          <div>
            <h3 className="text-2xl font-bold text-zinc-900 dark:text-zinc-100">{offer.name}</h3>
            <p className="text-zinc-600 dark:text-zinc-400 mt-1">{offer.tagline}</p>
          </div>
          <div className="text-right flex-shrink-0">
            <div className="text-3xl font-bold text-zinc-900 dark:text-zinc-100">{offer.price}</div>
            <p className="text-sm text-zinc-600 dark:text-zinc-400">{offer.paymentPlan}</p>
          </div>
        </div>
        ...
        <ul className="space-y-2.5 mb-6">
          {keyBullets.slice(0, 6).map((bullet, i) => (
            <li key={i} className="flex items-start gap-3">
              <div className="flex-shrink-0 w-5 h-5 rounded-full bg-zinc-100 dark:bg-zinc-700/50 flex items-center justify-center mt-0.5">
        ...
        <button
          ...
          className="flex items-center gap-2 text-violet-600 hover:text-violet-500 dark:text-violet-400 dark:hover:text-violet-300 transition-colors text-sm font-medium mb-6"
        >
        ...
        {expanded && (
          <div className="space-y-8 mb-6 border-t border-zinc-200 dark:border-zinc-800 pt-6">
            ...
            <div>
              ...
              <div className="space-y-4">
                {offer.weeks.map((week) => (
                  <div
                    key={week.week}
                    className="border-l-2 border-violet-200 dark:border-violet-500/30 pl-4"
                  >
            ...
            {/* Guarantee */}
            <div className="bg-zinc-50 dark:bg-zinc-800/50 rounded-lg p-4 flex gap-3">
              <Shield className="w-5 h-5 text-violet-400 flex-shrink-0 mt-0.5" />
              <div>
                <p className="font-medium text-zinc-800 dark:text-zinc-200">{offer.guarantee}</p>
                <p className="text-sm text-zinc-600 dark:text-zinc-400 mt-1">
                  {offer.guaranteeDetails}
                </p>
              </div>
            </div>
          </div>
        ...
        {paymentUrl ? (
          ...
          <button
            disabled
            className="block w-full py-4 px-6 rounded-lg font-semibold text-center bg-zinc-100 text-zinc-400 dark:bg-zinc-800 dark:text-zinc-500 cursor-not-allowed"
          >
            Coming Soon
          </button>
        )}

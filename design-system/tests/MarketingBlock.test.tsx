...
describe('MarketingBlock', () => {
  ...
  it('renders null when isVisible is false', () => {
    const { container } = render(<MarketingBlock block={makeBlock({ isVisible: false })} />);
    expect(container.firstChild).toBeNull();
  });
  ...
  it('renders bold text in content', () => {
    render(<MarketingBlock block={makeBlock({ content: 'This is **bold** text.' })} />);
    const bold = screen.getByText('bold');
    expect(bold.tagName).toBe('STRONG');
  });

  it('renders italic text in content', () => {
    render(<MarketingBlock block={makeBlock({ content: 'This is *italic* text.' })} />);
  ...
  it('applies custom className', () => {
    const { container } = render(<MarketingBlock block={makeBlock()} className="mt-4" />);

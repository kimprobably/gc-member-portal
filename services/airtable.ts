...
export async function verifyUser(email: string): Promise<User | null> {
  try {
    ...
    const res = await fetch(`https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/${USERS_TABLE}?filterByFormula=${filter}`, {
...
export async function fetchCourseData(targetCohort: string, userEmail: string): Promise<CourseData> {
  try {
    const [lessonsRes, actionsRes] = await Promise.all([
        fetch(`https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/${encodeURIComponent(LESSONS_TABLE)}?view=Grid%20view`, {
            headers: { Authorization: `Bearer ${AIRTABLE_API_KEY}` }
        }),
        fetch(`https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/${encodeURIComponent(ACTION_ITEMS_TABLE)}?view=Grid%20view`, {
            headers: { Authorization: `Bearer ${AIRTABLE_API_KEY}` }
        })
    ]);
    ...
    lessonsData.records.forEach((record: any) => {
        ...
        if (!weeksMap.has(weekName)) {
            weeksMap.set(weekName, { id: weekName.toLowerCase().replace(/\s+/g, '-'), title: weekName, lessons: [], actionItems: [] });
        }
